...
export function VerifierSidebar() {
  ...
  const { recentProjects } = useRecentProjects('verifier', 3)

  const rolePrefix = '/verifier'

  // Check if we're on a project detail page
  const projectDetailMatch = pathname?.match(/^\/verifier\/projects\/([^/]+)/)
  ...
  return (
    <Sidebar>
      ...
      <SidebarBody>
        <SidebarSection>
          <SidebarItem href="/verifier" current={pathname === '/verifier'}>
            <HomeIcon />
