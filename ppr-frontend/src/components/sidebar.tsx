...
export function Sidebar({ className, ...props }: React.ComponentPropsWithoutRef<'nav'>) {
  return <nav {...props} className={clsx(className, 'flex h-full min-h-0 flex-col overflow-x-hidden')} />
}
...
export function SidebarGroup({ defaultOpen = true, button, children, className }: SidebarGroupProps) {
  const [isOpen, setIsOpen] = React.useState(defaultOpen)

  return (
    <div className={clsx(className, 'flex min-w-0 flex-col gap-0.5')}>
      {React.isValidElement(button)
        ...
        : button}
      {isOpen && <div className="flex min-w-0 flex-col gap-0.5">{children}</div>}
    </div>
  )
}
    ...
    <span className={clsx(className, 'relative')}>
      ...
      {href ? (
        <Headless.CloseButton
          as={Link}
          href={href}
          className={clsx('cursor-pointer', buttonClasses)}
          data-current={current ? 'true' : undefined}
          onClick={onToggle}
        >
...
export function SidebarSubItem({
  ...
  let classes = clsx(
    // Base
    'flex min-w-0 items-center gap-3 rounded-lg px-2 py-2 text-left text-sm/5 font-medium text-zinc-950 transition-colors duration-150',
    // Indentation for sub-items - use padding instead of margin to avoid overflow
    'pl-8',
    // Leading icon/icon-only
    ...
    'dark:data-active:bg-white/5 dark:data-active:*:data-[slot=icon]:fill-white',
    'dark:data-current:*:data-[slot=icon]:fill-white',
    // Merge custom className for nested items
    className
  )
